...
const LearningActivity = () => {
  ...
  const TOTAL_TARGET = 5; // Target for progress bar (ADHD learners)
  ...
  const getFallbackActivity = (): ActivityItem | null => {
    try {
      console.log('Getting fallback activity...');
      const byId = getActivityById('M1_L1_Q1');
      console.log('Activity by ID M1_L1_Q1:', byId ? 'found' : 'not found');
      
      if (byId) return byId;
      
      const m1 = getActivitiesByLesson('M1', '1.1');
      console.log('M1 L1.1 activities:', m1.length);
      if (m1.length > 0) return m1[0];
      
      const m2 = getActivitiesByLesson('M2', '2.1');
      console.log('M2 L2.1 activities:', m2.length);
      if (m2.length > 0) return m2[0];
      
      const m3 = getActivitiesByLesson('M3', '3.2');
      console.log('M3 L3.2 activities:', m3.length);
      if (m3.length > 0) return m3[0];
      
      console.error('No fallback activities found in any lesson!');
      return null;
    } catch (e) {
  ...
  const loadNext = async () => {
    ...
    let apiWorked = false;
    try {
      console.log('Attempting to fetch activity from API...');
  ...
  const hasADHD = user?.neuroFlags?.includes('ADHD') || user?.neurodiversityTags?.includes('ADHD');
  const showProgressBar = hasADHD && activity.accessibility.showProgressBar;
    ...
    <div className={cn('min-h-screen bg-background', distractionFree && 'distraction-free')}>
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          {/* Progress Bar for ADHD learners */}
          {showProgressBar && (
            <Card className="p-4">
              <ADHDProgressBar
